...
short OutputThread::buf[MM_NUM_OUTPUT_BUFFERS]
                       [MM_SOUND_BUFFER_LEN];  // это ~25 мс при 16кГц

static volatile bool flag_ShutDownOutputThread =
    false;  // флаг информирует поток о том, что нужно выключиться
static volatile bool flag_DoNotWriteBuffers =
...
//===============================================================================
// Создаёт поток и начинает выкладывать блоки данных на устройство
// воспроизведения по умолчанию HWND нужно для привязки сообщений об ошибках к
// родительскому окну
//===============================================================================
int OutputThread::Start(HWND hdwnd) {
  ...
  // 2. Если использовали устройство - закончить его использовать (быть того не
  // может!)
  if (device) {
    CleanUpDevice(hdwnd);
...
int OutputThread::OpenDevice(HWND hdwnd) {
  ...
  mres = waveOutOpen(
      &device, 0, &WaveFormat, (DWORD)hOutputSoundDataReady, 0, CALLBACK_EVENT);
  if (mres) {
    MessageBox(hdwnd,
               L"Формат воспроизведения 16кГц 16 бит НЕ подддерживается!",
               L"Поддержка формата",
               MB_OK);
    return 1;
  }
  ...
  for (i = 0; i < MM_NUM_OUTPUT_BUFFERS; i++) {
    ...
    if (mres) {
      MessageBox(hdwnd,
                 L"не отработал waveOutPrepareHeader",
                 L"не отработал waveOutPrepareHeader",
                 MB_OK);
      return 1;
    }
...
void OutputThread::CleanUpDevice(HWND hdwnd) {
  ...
  if (device) {
    // Первым делом нужно, чтобы поток, который выстреливает звук, перестал это
    // делать!
    ...
    for (i = 0; i < MM_NUM_OUTPUT_BUFFERS; i++) {
      ...
      if (mres)
        MessageBox(hdwnd,
                   L"не отработал waveOutUnprepareHeader",
                   L"не отработал waveOutUnprepareHeader",
                   MB_OK);
      ZeroMemory(&WaveHeader[i],
                 sizeof(WaveHeader[i]));  // Вот без этого повторная
                                          // инициализация глючила, потому что
      // Event приходил ещё до того, как были подготовлены буферы, и OnSoundData
      // поперёк батьки делала waveOutWrite()
    }

    mres = waveOutClose(device);
    if (mres)
      MessageBox(hdwnd,
                 L"не отработал waveOutClose",
                 L"не отработал waveOutClose",
                 MB_OK);
...
#ifdef _DEBUG
  // А был ли буфер?
  if (0 == duplex) OutputDebugString(L"BAD FLAGS\r\n");
#endif
}
