...
bool MHKeypad::Press4(int position, bool down, int shift)
{
	...
	if (0xFFFF != scancode[position + shift]) {
		// Обработка кликов мыши (ЛКМ и ПКМ)
		if (scancode[position + shift] == 0xE110 || scancode[position + shift] == 0xE111)
		{
			input1.type = INPUT_MOUSE;
			if (scancode[position + shift] == 0xE110)
			{
				input1.mi.dwFlags = down ? MOUSEEVENTF_LEFTDOWN : MOUSEEVENTF_LEFTUP;
			}
			else
			{
				input1.mi.dwFlags = down ? MOUSEEVENTF_RIGHTDOWN : MOUSEEVENTF_RIGHTUP;
			}
	...
	if (position == 5 || position == 10) {
		...
		if (0xFFFF != scancode[second_key_index]) {
			// Обработка кликов мыши для второй клавиши (ЛКМ и ПКМ)
			if (scancode[second_key_index] == 0xE110 || scancode[second_key_index] == 0xE111)
			{
				input2.type = INPUT_MOUSE;
				if (scancode[second_key_index] == 0xE110)
				{
					input2.mi.dwFlags = down ? MOUSEEVENTF_LEFTDOWN : MOUSEEVENTF_LEFTUP;
				}
				else
				{
					input2.mi.dwFlags = down ? MOUSEEVENTF_RIGHTDOWN : MOUSEEVENTF_RIGHTUP;
				}
