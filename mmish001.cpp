...
// Потом это вынесем куда-нибудь. Этим пользуются все - и WorkerThread, и
// CopyShmopy, и Indicators
...
volatile bool flag_keep_silence =
    true;  // [27-DEC] - отмена отключения устройства воспроизведения. звук
           // нужен для кликов. теперь обнуляем буфер, когда нужна тишина.
...
static int training_buttons[MM_NUM_TRAINING_BUTTONS] = {IDC_BUTTON_TRAIN0,
                                                        IDC_BUTTON_TRAIN1,
                                                        IDC_BUTTON_TRAIN2,
                                                        IDC_BUTTON_TRAIN3,
                                                        IDC_BUTTON_TRAIN4,
                                                        IDC_BUTTON_TRAIN5};
static int exclaim_controls[MM_NUM_TRAINING_BUTTONS] = {IDC_STATIC_BMP0,
                                                        IDC_STATIC_BMP1,
                                                        IDC_STATIC_BMP2,
                                                        IDC_STATIC_BMP3,
                                                        IDC_STATIC_BMP4,
...
MHWORDChar dlg_scancodes[MH_NUM_SCANCODES] = {
    ...
    {L"F6", 0x40},
    {L"F7", 0x41},
    {L"(F8 - disabled) ", 0xFFFF},  // 70-74
    {L"F9", 0x43},
    {L"F10", 0x44},
    {L"F11", 0x57},
    {L"F12", 0x58},
    {L"Scroll Lock", 0x46},  // 75-79
    {L"Insert", 0xE052},
    {L"(Delete - disabled)", 0xE053},
    {L"Home", 0xE047},
    {L"End", 0xE04F},
    {L"PgUp", 0xE049},  // 80-84
    {L"PgDn", 0xE051},
    ...
    {L"Left mouse button", 0xFF00},
    {L"Right mouse button", 0xFF01}};
#else
MHWORDChar dlg_scancodes[MH_NUM_SCANCODES] = {
    {L"<ничего>", 0xFFFF},  // 0
    {L"вверх", 0xE048},
    ...
    {L"Левый Ctrl", 0x1D},
    {L"Левый Alt", 0x38},
    {L"Левый Win", 0xE05B},  // 55-59
    {L"Правый Shift", 0x36},
    {L"Правый Ctrl", 0xE01D},
    {L"Правый Alt", 0xE038},
    {L"Правый WIN", 0xE05C},
    {L"Menu", 0xE05D},  // 60-64
    {L"Esc", 0x01},
...
static void SetKeysInDialogue() {
  ...
  int listbox2[4] = {IDC_CHECK_REPEAT0,
                     IDC_CHECK_REPEAT1,
                     IDC_CHECK_REPEAT2,
                     IDC_CHECK_REPEAT3};
  int listbox3[4] = {IDC_CHECK_TOGGLE0,
                     IDC_CHECK_TOGGLE1,
                     IDC_CHECK_TOGGLE2,
                     IDC_CHECK_TOGGLE3};
  int i, j;
    ...
    for (j = 0; j < MH_NUM_SCANCODES; j++) {
      if (key == dlg_scancodes[j].value) {
        ...
        SendDlgItemMessage(
            ...
            CB_SETCURSEL,
            j,
            0L);  // Скорректируем выбранное устройство в списке диалога
        break;
      }
    }
...
// Возвращает признак того, что модель может работать (используется при запуске
// программы)
//============================================================================================
bool UpdateExclaim(bool hide = false) {
  ...
  if (!hbm_exclaim)
    hbm_exclaim = (HBITMAP)LoadImage(GZInst,
  ...
  for (i = 0; i < MM_NUM_TRAINING_BUTTONS; i++) {
    if ((model.IsSoundFilled(i)) || hide) {
      ...
      SendDlgItemMessage(
          hdwnd, exclaim_controls[i], STM_SETIMAGE, IMAGE_BITMAP, NULL);
    } else {
...
#ifdef MMISH_ENGLISH
    SetDlgItemText(hdwnd,
                   IDC_STATIC_GROM,
                   L"Untrained. To enable, press \"Train\", then pause, then "
                   L"make your sound several times.");
#else
    SetDlgItemText(hdwnd,
                   IDC_STATIC_GROM,
                   L"Программа ещё не обучена некоторым звукам. Для обучения "
                   L"нажмите кнопку “Тренировать” и, выждав паузу, несколько "
                   L"раз произнесите в микрофон нужный звук.");
#endif
    return false;
  } else {
    // убираем лишнее
    SendDlgItemMessage(
        hdwnd, IDC_STATIC_BMP6, STM_SETIMAGE, IMAGE_BITMAP, NULL);
...
void StopTrainingMode() {
  ...
  // Показать все кнопки кроме той, что с номером _sound. Ей поменять надпись на
  // "Тренировать"
  for (i = 0; i < MM_NUM_TRAINING_BUTTONS; i++) {
    if (i == training_sound)
#ifdef MMISH_ENGLISH
      ...
      EnableWindow(GetDlgItem(hdwnd, training_buttons[i]), TRUE);
  }
...
void StartTrainingMode(int _sound) {
  ...
  // Спрятать все кнопки кроме той, что с номером _sound. Ей поменять надпись на
  // "Прекратить"
  for (i = 0; i < MM_NUM_TRAINING_BUTTONS; i++) {
    if (i == _sound)
#ifdef MMISH_ENGLISH
      ...
      EnableWindow(GetDlgItem(hdwnd, training_buttons[i]), FALSE);
  }
  ...
  flag_training_mode =
      true;  // Важен порядок, ибо другой поток сначала читает flag...
  ...
  SetDlgItemText(
      hdwnd,
      IDC_STATIC_GROM,
      L"Speak louder than the noise level (marked with the red line)");
#else
  SetDlgItemText(hdwnd,
                 IDC_STATIC_GROM,
                 L"Говорите громче уровня шума (отмечен красной полоской)");
#endif
...
static BOOL CALLBACK DlgWndProc(HWND hdwnd,
  ...
  switch (uMsg) {
    ...
    case WM_INITDIALOG:
    {
      ...
      // 2.1 Проверить, есть ли вообще звуковые устройства. Ибо если нет -
...
#ifdef MMISH_ENGLISH
      SendDlgItemMessage(
          hdwnd, IDC_COMBO_MIC, CB_ADDSTRING, 0, (LPARAM)L"Reading from file");
#else
      SendDlgItemMessage(
          hdwnd, IDC_COMBO_MIC, CB_ADDSTRING, 0, (LPARAM)L"Чтение из файла");
#endif
      ...
      // Теперь воспроизведение через звуковую карту бывает только для файлов в
      // Release-версии [27-DEC] - отмена. звук нужен для кликов. теперь
      // обнуляем буфер, когда нужна тишина.
      //#ifdef _DEBUG
    ...
    case WM_COMMAND:
      switch (LOWORD(wparam)) {
        ...
        case IDC_BUTTON_SAVE:
          if (model.Save(false, hdwnd, L"default.MM1"))
        ...
        case IDC_BUTTON_LOAD:
          ...
          {
                  flag_model_changed=false;
                  if(KChFstate::flag_kc_anytime) // Корректируем галочку в
          диалоге SendDlgItemMessage(hdwnd, IDC_CHECK_IGNORE_KC_INLINE,
          BM_SETCHECK, BST_UNCHECKED, 0); else SendDlgItemMessage(hdwnd,
          IDC_CHECK_IGNORE_KC_INLINE, BM_SETCHECK, BST_CHECKED, 0);
                  InputThread::Start(current_device_num,hdwnd); // выставить
          устройство, прописанное в файле EnableWindow( GetDlgItem( hdwnd,
        ...
        case IDC_CHECK_IGNORE_KC_INLINE:
          flag_model_changed = true;
          // Тут можно было как-то wparam контролировать, но не было под рукой
          // справки
          if (BST_CHECKED
              == SendDlgItemMessage(
                  hdwnd, IDC_CHECK_IGNORE_KC_INLINE, BM_GETCHECK, 0, 0))
        ...
        case IDC_CHECK_REPEAT0:
          ...
          if (BST_CHECKED
              == SendDlgItemMessage(
                  hdwnd, IDC_CHECK_REPEAT0, BM_GETCHECK, 0, 0))
        ...
        case IDC_CHECK_REPEAT1:
          ...
          if (BST_CHECKED
              == SendDlgItemMessage(
                  hdwnd, IDC_CHECK_REPEAT1, BM_GETCHECK, 0, 0))
        ...
        case IDC_CHECK_REPEAT2:
          ...
          if (BST_CHECKED
              == SendDlgItemMessage(
                  hdwnd, IDC_CHECK_REPEAT2, BM_GETCHECK, 0, 0))
        ...
        case IDC_CHECK_REPEAT3:
          ...
          if (BST_CHECKED
              == SendDlgItemMessage(
                  hdwnd, IDC_CHECK_REPEAT3, BM_GETCHECK, 0, 0))
        ...
        case IDC_CHECK_TOGGLE0:
          ...
          if (BST_CHECKED
              == SendDlgItemMessage(
                  hdwnd, IDC_CHECK_TOGGLE0, BM_GETCHECK, 0, 0))
        ...
        case IDC_CHECK_TOGGLE1:
          ...
          if (BST_CHECKED
              == SendDlgItemMessage(
                  hdwnd, IDC_CHECK_TOGGLE1, BM_GETCHECK, 0, 0))
        ...
        case IDC_CHECK_TOGGLE2:
          ...
          if (BST_CHECKED
              == SendDlgItemMessage(
                  hdwnd, IDC_CHECK_TOGGLE2, BM_GETCHECK, 0, 0))
        ...
        case IDC_CHECK_TOGGLE3:
          ...
          if (BST_CHECKED
              == SendDlgItemMessage(
                  hdwnd, IDC_CHECK_TOGGLE3, BM_GETCHECK, 0, 0))
        ...
        case IDC_KBD0:
          ...
          {
            KChFstate::SetKeyToPress(
                0,
                dlg_scancodes[SendDlgItemMessage(
                                  hdwnd, IDC_KBD0, CB_GETCURSEL, 0, 0L)]
                    .value);
            flag_model_changed = true;
            return 1;
          } else
        ...
        case IDC_KBD1:
          if (CBN_SELCHANGE == HIWORD(wparam))  // Выбрали другую клавишу
          {
            KChFstate::SetKeyToPress(
                1,
                dlg_scancodes[SendDlgItemMessage(
                                  hdwnd, IDC_KBD1, CB_GETCURSEL, 0, 0L)]
                    .value);
            flag_model_changed = true;
            return 1;
          } else
        ...
        case IDC_KBD2:
          if (CBN_SELCHANGE == HIWORD(wparam))  // Выбрали другую клавишу
          {
            KChFstate::SetKeyToPress(
                2,
                dlg_scancodes[SendDlgItemMessage(
                                  hdwnd, IDC_KBD2, CB_GETCURSEL, 0, 0L)]
                    .value);
            flag_model_changed = true;
            return 1;
          } else
        ...
        case IDC_KBD3:
          if (CBN_SELCHANGE == HIWORD(wparam))  // Выбрали другую клавишу
          {
            KChFstate::SetKeyToPress(
                3,
                dlg_scancodes[SendDlgItemMessage(
                                  hdwnd, IDC_KBD3, CB_GETCURSEL, 0, 0L)]
                    .value);
            flag_model_changed = true;
            return 1;
          } else
        ...
        case IDC_KBD4:
          if (CBN_SELCHANGE == HIWORD(wparam))  // Выбрали другую клавишу
          {
            KChFstate::SetKeyToPress(
                4,
                dlg_scancodes[SendDlgItemMessage(
                                  hdwnd, IDC_KBD4, CB_GETCURSEL, 0, 0L)]
                    .value);
            flag_model_changed = true;
            return 1;
          } else
        ...
        case IDC_KBD5:
          if (CBN_SELCHANGE == HIWORD(wparam))  // Выбрали другую клавишу
          {
            KChFstate::SetKeyToPress(
                5,
                dlg_scancodes[SendDlgItemMessage(
                                  hdwnd, IDC_KBD5, CB_GETCURSEL, 0, 0L)]
                    .value);
            flag_model_changed = true;
            return 1;
          } else
...
// Командную строку не обрабатываем
{
  ...
  // Диалог теперь - немодальный. Ибо в режиме суперпользователя появляется ещё
  // одно окно
  // DialogBox(hInst,MAKEINTRESOURCE(IDD_DIALOG1),NULL,(DLGPROC)DlgWndProc);
#ifdef MMISH_ENGLISH
  hdwnd = CreateDialog(
      hInst, MAKEINTRESOURCE(IDD_DIALOG1_ENG), NULL, (DLGPROC)DlgWndProc);
#else
  hdwnd = CreateDialog(
      hInst, MAKEINTRESOURCE(IDD_DIALOG1), NULL, (DLGPROC)DlgWndProc);
#endif
  ...
  if (model.Load(NULL, L"default.MM1")) {
    ...
    SendDlgItemMessage(
        ...
        current_device_num,
        0L);  // Скорректируем выбранное устройство в списке диалога
    InputThread::Start(current_device_num,
                       hdwnd);  // выставить устройство, прописанное в файле
  }

  // При необходимости напечатать восклицательные знаки
  // Если же модель полностью натренирована, разрешаем работу и пишем на кнопке
  // "Старт" 05-jun-2019 - сразу никогда не запускаем
  if (UpdateExclaim()) {
    // flag_move_mouse=true;
    // SetDlgItemText(hdwnd, IDOK, L"Стоп");
  }
