...
class InputThread {
 public:
  static int Start(UINT device_num,
                   HWND hdwnd);  // Создаёт поток и начинает заглатывать блоки
                                 // данных с устройства записи звука
  static void Halt(HWND hdwnd);  // Чистим за собой
  static void OnSoundData();  // В поток пришла повесточка от звуковой карты

 protected:
  static int OpenDevice(UINT device_num,
                        HWND hdwnd);  // Начинаем работу со звуковой картой
